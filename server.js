// server.js (новые WebSocket-события + автосохранение сцены)
...
app.use(cors({ origin: 'https://apostol.onrender.com', allowedHeaders: ['Content-Type', 'Authorization'] }));
...
const schemas = {
  ...
  uploadFile: Joi.object({ image: Joi.string().max(15 * 1024 * 1024).required(), name: Joi.string().required(), campaign_id: Joi.string().uuid() }),
};
...
async function enrichCharacter(ch) {
  ...
  const enrichSingle = async (char) => {
    ...
    const skillMap = {};
    for (const p of (perks || [])) { for (const m of (p.effect_modifiers || [])) { skillMap[m.skill] = (skillMap[m.skill] || 0) + (m.modifier || 0); } }
    const enrichedSkills = (skills || []).map(s => { const baseMod = (cs || []).find(e => e.skill_id === s.id)?.modifier || 0; const perkBonus = skillMap[s.name] || 0; return { ...s, modifier: baseMod, baseModifier: baseMod, perkBonus, totalModifier: baseMod + perkBonus, totalPercent: baseMod + perkBonus }; });
...
function notifyCampaign(campaignId, event, data) {
...
io.on('connection', (socket) => {
  ...
  socket.on('dice_roll', (data) => {
    ...
    if (data.hidden) { const room = io.sockets.adapter.rooms.get(`campaign:${data.campaignId}`); if (room) for (const sid of room) { const s = io.sockets.sockets.get(sid); if (s?.data?.role && ['master','co-master'].includes(s.data.role)) s.emit('dice_result', payload); } socket.emit('dice_result', payload); }
...
app.post('/api/inventory/remove', authMiddleware, async (req, res) => {
  ...
  const { data: slot } = await supabase.from('inventory_slots').select('*, character:characters(campaign_id, user_id)').eq('id', slot_id).single();
  ...
  else { await supabase.from('inventory_slots').update({ quantity: nq }).eq('id', slot_id).select('*, item:items(*)').single(); }
...
app.post('/api/inventory/equip', authMiddleware, async (req, res) => {
  ...
  if (!ch || ch.user_id !== req.user.id) return res.status(403).json({ error: 'Не ваш персонаж' });
  const item = slot.item; const cid = slot.character_id;
  if (item.is_weapon) {
...
app.post('/api/inventory/use', authMiddleware, async (req, res) => {
  ...
  if (!ch || ch.user_id !== req.user.id) return res.status(403).json({ error: 'Не ваш персонаж' });
  const item = slot.item; let result = null;
  if (item.weapon_type === 'ranged' && slot.equipped) { if ((item.current_ammo || 0) <= 0) return res.status(400).json({ error: 'Нет патронов' }); await supabase.from('items').update({ current_ammo: (item.current_ammo || 1) - 1 }).eq('id', item.id); result = { used: 'ammo', remaining: (item.current_ammo || 1) - 1, action: 'выстрелил из' }; }
  else if (item.weapon_type === 'thrown' && slot.equipped) { await supabase.from('inventory_slots').delete().eq('id', slot_id); result = { used: 'thrown', deleted: true, action: 'метнул' }; }
  else if (item.type === 'расходник') { const nq = slot.quantity - 1; if (nq <= 0) { await supabase.from('inventory_slots').delete().eq('id', slot_id); result = { used: 'consumable', deleted: true, action: 'использовал' }; } else { await supabase.from('inventory_slots').update({ quantity: nq }).eq('id', slot_id); result = { used: 'consumable', remaining: nq, action: 'использовал' }; } }
  else return res.status(400).json({ error: 'Нельзя использовать' });
  if (ch.campaign_id) { const msgText = `${ch.name} ${result.action} ${item.name}`; const { data: msg } = await supabase.from('chat_messages').insert({ campaign_id: ch.campaign_id, user_id: req.user.id, username: ch.name, text: msgText, is_roll: false }).select().single(); if (msg) notifyCampaign(ch.campaign_id, 'chat_message', msg); notifyCampaign(ch.campaign_id, 'inventory_updated', { character_id: slot.character_id }); }
  res.json(result);
});
app.post('/api/inventory/reload', authMiddleware, async (req, res) => {
  ...
  const { data: ch } = await supabase.from('characters').select('user_id, campaign_id').eq('id', slot.character_id).single();
  if (!ch || ch.user_id !== req.user.id) return res.status(403).json({ error: 'Не ваш персонаж' });
  const neededAmmoType = slot.item.ammo_type; if (!neededAmmoType) return res.status(400).json({ error: 'Для этого оружия не указан тип патронов' });
  const maxAmmo = slot.item.max_ammo || 0; const currentAmmo = slot.item.current_ammo || 0; const needed = maxAmmo - currentAmmo;
  ...
  const ammoSlot = allSlots?.find(s => s.item?.ammo_type === neededAmmoType); if (!ammoSlot) return res.status(400).json({ error: `Нет патронов типа "${neededAmmoType}"` });
  const ammoAvailable = ammoSlot.quantity; const toReload = Math.min(needed, ammoAvailable);
  await supabase.from('items').update({ current_ammo: currentAmmo + toReload }).eq('id', slot.item.id);
  const remaining = ammoAvailable - toReload; if (remaining <= 0) await supabase.from('inventory_slots').delete().eq('id', ammoSlot.id); else await supabase.from('inventory_slots').update({ quantity: remaining }).eq('id', ammoSlot.id);
  if (ch.campaign_id) notifyCampaign(ch.campaign_id, 'inventory_updated', { character_id: slot.character_id });
...
app.put('/api/inventory/:slotId', authMiddleware, async (req, res) => {
  ...
  const updates = {}; if (condition_percent !== undefined) updates.condition_percent = condition_percent; if (equipped !== undefined) updates.equipped = equipped; if (slot_type !== undefined) updates.slot_type = slot_type; if (quantity !== undefined) updates.quantity = quantity;
  const { data, error } = await supabase.from('inventory_slots').update(updates).eq('id', req.params.slotId).select('*, item:items(*)').single();
  if (error) return res.status(500).json({ error: error.message });
  if (data?.character_id) { const { data: ch } = await supabase.from('characters').select('campaign_id').eq('id', data.character_id).single(); if (ch?.campaign_id) notifyCampaign(ch.campaign_id, 'inventory_updated', { character_id: data.character_id }); }
  res.json(data);
});
...
app.post('/api/inventory/:slotId/mod', authMiddleware, async (req, res) => {
  ...
  const mods = slot.mods || []; if (mods.find(m => m.id === modItem.id)) return res.status(400).json({ error: 'Модификация уже установлена' });
...
app.get('/api/npcs', authMiddleware, async (req, res) => {
  ...
  let q = supabase.from('npcs').select('*'); if (campaign_id) q = q.eq('campaign_id', campaign_id); if (is_template) q = q.eq('is_template', true);
  const { data } = await q;
  res.json(data || []);
});
...
app.put('/api/npcs/:id', authMiddleware, async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: error.message });
  const campaignId = data?.campaign_id || existing?.campaign_id;
  if (campaignId) notifyCampaign(campaignId, 'npcs_updated', { campaign_id: campaignId });
  res.json(data);
});
...
app.post('/api/npcs/:id/roll', authMiddleware, validate(schemas.npcRoll), async (req, res) => {
  ...
  const mod = skill.modifier || 0; const d20 = Math.floor(Math.random()*20)+1;
  res.json({ npc_name: npc.name, skill_name: req.body.skill_name, d20roll: d20, modifier: mod, sum: d20+mod, formula: `d20 (${d20}) + ${mod}` });
});

// ===== ITEMS =====
app.get('/api/items', authMiddleware, async (req, res) => { const { data } = await supabase.from('items').select('*'); res.json(data); });
app.post('/api/items', authMiddleware, async (req, res) => { const { data, error } = await supabase.from('items').insert(req.body).select().single(); if (error) return res.status(500).json({ error: error.message }); res.json(data); });
...
app.post('/api/dice/auto', authMiddleware, diceLimiter, validate(schemas.diceAuto), async (req, res) => {
  ...
  if (!ch) return res.status(404).json({ error: 'Не найден' });
  const [{ data: cs }, { data: cp }, { data: skills }] = await Promise.all([ supabase.from('character_skills').select('skill_id, modifier').eq('character_id', character_id), supabase.from('character_perks').select('perk_id').eq('character_id', character_id), supabase.from('skills').select('*') ]);
  const skill = skills?.find(s => s.name === skill_name); if (!skill) return res.status(404).json({ error: 'Навык не найден' });
  const baseModifier = (cs || []).find(e => e.skill_id === skill.id)?.modifier || 0;
  const pIds = (cp || []).map(x => x.perk_id); let perkBonus = 0; if (pIds.length) { const { data: perks } = await supabase.from('perks').select('*').in('id', pIds); for (const p of (perks || [])) for (const m of (p.effect_modifiers||[])) if (m.skill === skill_name) perkBonus += m.modifier||0; }
  const totalPercent = baseModifier + perkBonus; const d20 = Math.floor(Math.random() * 20) + 1; const bonus = Math.round(20 * totalPercent / 100); const sum = d20 + bonus;
...
app.put('/api/scenes/:campaign_id', authMiddleware, validate(schemas.updateScene), async (req, res) => {
  ...
  if (existing) { const updates = {}; if (background_url !== undefined) updates.background_url = background_url; if (fog_of_war !== undefined) updates.fog_of_war = fog_of_war; if (tokens !== undefined) updates.tokens = tokens; if (drawings !== undefined) updates.drawings = drawings; if (portals !== undefined) updates.portals = portals; const { data, error } = await supabase.from('scenes').update(updates).eq('id', existing.id).select().single(); if (error) return res.status(500).json({ error: error.message }); return res.json(data); }
  else { const { data, error } = await supabase.from('scenes').insert({ campaign_id: req.params.campaign_id, scene_type, background_url: background_url || null, fog_of_war: fog_of_war || [], tokens: tokens || [], drawings: drawings || [], portals: portals || [] }).select().single(); if (error) return res.status(500).json({ error: error.message }); return res.json(data); }
});

// ===== BACKGROUNDS =====
app.post('/api/upload/background', authMiddleware, async (req, res) => { const { campaign_id, name, url } = req.body; if (!campaign_id || !url) return res.status(400).json({ error: 'campaign_id и url обязательны' }); const { data, error } = await supabase.from('backgrounds').insert({ campaign_id, name, url }).select().single(); if (error) return res.status(500).json({ error: error.message }); res.json(data); });
app.get('/api/backgrounds/:campaign_id', authMiddleware, async (req, res) => { const { data } = await supabase.from('backgrounds').select('*').eq('campaign_id', req.params.campaign_id); res.json(data || []); });
...
app.delete('/api/handouts/:id', authMiddleware, async (req, res) => { const { data: handout } = await supabase.from('handouts').select('campaign_id').eq('id', req.params.id).single(); await supabase.from('handouts').delete().eq('id', req.params.id); if (handout?.campaign_id) notifyCampaign(handout.campaign_id, 'handouts_updated', { campaign_id: handout.campaign_id }); res.json({ success: true }); });

// ===== СОУНДПАД =====
app.get('/api/sounds/:campaign_id', authMiddleware, async (req, res) => { const { data } = await supabase.from('sounds').select('*').or(`campaign_id.eq.${req.params.campaign_id},is_global.eq.true`).order('name', { ascending: true }); res.json(data || []); });
app.post('/api/sounds', authMiddleware, async (req, res) => { const { campaign_id, name, file_url, source_type, duration, category } = req.body; const { data, error } = await supabase.from('sounds').insert({ campaign_id, name, file_url, source_type: source_type || 'url', duration: duration || 0, category: category || 'общее', is_global: false }).select().single(); if (error) return res.status(500).json({ error: error.message }); if (data?.campaign_id) notifyCampaign(data.campaign_id, 'sounds_updated', { campaign_id: data.campaign_id }); res.json(data); });
app.delete('/api/sounds/:id', authMiddleware, async (req, res) => { const { data: sound } = await supabase.from('sounds').select('campaign_id').eq('id', req.params.id).single(); await supabase.from('sounds').delete().eq('id', req.params.id); if (sound?.campaign_id) notifyCampaign(sound.campaign_id, 'sounds_updated', { campaign_id: sound.campaign_id }); res.json({ success: true }); });

// ===== АДМИН-ПАНЕЛЬ =====
app.get('/api/admin/items', authMiddleware, adminMiddleware, async (req, res) => { const { data } = await supabase.from('items').select('*').order('name'); res.json(data || []); });
...
app.get('/api/admin/skills', authMiddleware, adminMiddleware, async (req, res) => { const { data } = await supabase.from('skills').select('*').order('name'); res.json(data || []); });
app.put('/api/admin/skills/:id', authMiddleware, adminMiddleware, async (req, res) => { const { data, error } = await supabase.from('skills').update(req.body).eq('id', req.params.id).select().single(); if (error) return res.status(500).json({ error: error.message }); res.json(data); });
...
process.on('uncaughtException', (err) => { console.error('НЕОБРАБОТАННАЯ ОШИБКА:', err.message); });
process.on('unhandledRejection', (reason) => { console.error('НЕОБРАБОТАННЫЙ ПРОМИС:', reason); });
