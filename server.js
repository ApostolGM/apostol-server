...
app.post('/api/campaigns', authMiddleware, adminMiddleware, validate(schemas.createCampaign), async (req, res) => {
  const { title } = req.body; const invite_code = uuidv4().substring(0, 8);
...
app.get('/api/characters/:id/weight', authMiddleware, async (req, res) => {
  ...
  if (!ch) return res.status(404).json({ error: 'Персонаж не найден' });
  const { data: slots } = await supabase.from('inventory_slots').select('quantity, item:items(weight, slot)').eq('character_id', ch.id);
  let totalWeight = 0;
  for (const s of (slots||[])) { if (s.item?.slot !== 'currency') totalWeight += (s.item?.weight||0) * (s.quantity||1); }
...
app.post('/api/inventory/add', authMiddleware, async (req, res) => {
  ...
  const { data: item } = await supabase.from('items').select('slot').eq('id', item_id).single();
...
app.post('/api/inventory/reload', authMiddleware, async (req, res) => {
  ...
  const { data: allSlots } = await supabase.from('inventory_slots').select('*, item:items(*)').eq('character_id', slot.character_id).eq('equipped', false).eq('item.slot', 'ammo');
  ...
  res.json({ success: true, current_ammo: currentAmmo + toReload, max_ammo, used: toReload, remaining_ammo_in_inventory: Math.max(0, remaining) });
});
...
app.post('/api/inventory/:slotId/mod', authMiddleware, async (req, res) => {
  ...
  if (modItem.mod_target !== 'any' && modItem.mod_target !== slot.item?.slot) return res.status(400).json({ error: 'Модификация не подходит к этому типу предмета' });
  if (modItem.mod_target === 'weapon' && modItem.weapon_mod_subtype && modItem.weapon_mod_subtype !== 'any' && modItem.weapon_mod_subtype !== slot.item?.weapon_type) return res.status(400).json({ error: 'Модификация не подходит к этому типу оружия' });
  ...
  const { data, error } = await supabase.from('inventory_slots').update({ mods }).eq('id', req.params.slotId).select('*, item:items(*, ammo_type:ammo_types(*))').single();
  if (error) return res.status(500).json({ error: error.message });
...
app.get('/api/backgrounds', authMiddleware, async (req, res) => { const { data } = await supabase.from('backgrounds').select('*').or('is_global.eq.true').order('name'); res.json(data||[]); });
...
app.post('/api/shop/buy', authMiddleware, async (req, res) => {
  ...
  const { data: currencySlots } = await supabase.from('inventory_slots').select('*, item:items(*)').eq('character_id', character_id).eq('item.slot', 'currency');
  let totalCurrency = 0;
  for (const cs of (currencySlots||[])) totalCurrency += (cs.item?.trade_price||0) * cs.quantity;
  if (totalCurrency < totalPrice) return res.status(400).json({ error: `Недостаточно средств. Нужно ${totalPrice}, у вас ${totalCurrency}` });
  let remaining = totalPrice;
  for (const cs of (currencySlots||[])) {
    if (remaining <= 0) break;
    const unitPrice = cs.item?.trade_price||0;
    const toRemove = Math.min(cs.quantity, Math.ceil(remaining / unitPrice));
    remaining -= toRemove * unitPrice;
    if (toRemove >= cs.quantity) await supabase.from('inventory_slots').delete().eq('id', cs.id);
    else await supabase.from('inventory_slots').update({ quantity: cs.quantity - toRemove }).eq('id', cs.id);
  }
  ...
  if (error) return res.status(500).json({ error: error.message });
  if (ch.campaign_id) notifyCampaign(ch.campaign_id, 'inventory_updated', { character_id });
  res.json({ success: true, item: data, price: totalPrice });
});
