...
app.post('/api/shop/buy', authMiddleware, async (req, res) => {
  ...
  const { data, error } = await supabase.from('inventory_slots').insert({
    character_id, item_id, quantity: quantity || 1, slot_type: 'рюкзак', equipped: false
...
// ===== CHARACTERISTICS =====
app.get('/api/characteristics', authMiddleware, async (req, res) => {
  const { data } = await supabase.from('characteristics').select('*').order('name');
  res.json(data || []);
});

app.post('/api/characteristics', authMiddleware, adminMiddleware, async (req, res) => {
  const { name, short_name, description } = req.body;
  const { data, error } = await supabase.from('characteristics').insert({ name, short_name, description }).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});

app.put('/api/characteristics/:id', authMiddleware, adminMiddleware, async (req, res) => {
  const { data, error } = await supabase.from('characteristics').update(req.body).eq('id', req.params.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});

app.delete('/api/characteristics/:id', authMiddleware, adminMiddleware, async (req, res) => {
